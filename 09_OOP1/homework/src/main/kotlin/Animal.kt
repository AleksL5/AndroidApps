...
open class Animal(var energy: Int, var weight: Int, val maxAge: Int, val name: String) {
	...
	open fun proCreate(animal: Animal): Animal{
		val newAnimal = Animal(Random.nextInt(0,10), Random.nextInt(0,5), this.maxAge, this.name)
		println("$animal procreating new ${newAnimal.name} Age: ${newAnimal.age} Energy: ${newAnimal.energy} Weight: ${newAnimal.weight} MaxAge: ${newAnimal.maxAge} ")
		return newAnimal
	}
